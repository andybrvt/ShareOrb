...
class SearchedNewSidePanel extends React.Component{
  ...
  directToChats = () => {
    // Since we have a nav link we will just do a onclickon the list
    // just to clear out the events
    ...
    return(
      ...
      <List
        ...
        renderItem={item => (
          ...
          >
          <List.Item
          onClick = {() => this.directToChats()}
           className = {`chatItem ${item.id === parseInt(this.props.param.id) ? "current": ""}`}>
...
export default SearchedNewSidePanel;
