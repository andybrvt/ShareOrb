...
class suggestSuggested(generics.ListAPIView):
    serializer_class = serializers.SuggestedUserSerializer
    def get_queryset(self):
        list = []
        temp=(self.request.user.get_following())
...
class configSuggestedSuggest(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        focusList = []
        if(models.User.objects.filter(id = 5).count() > 0 and models.User.objects.filter(id = 2).count() > 0 ):
            andy = models.User.objects.get(id = 1)
            ping = models.User.objects.get(id = 2)
            jon = models.User.objects.get(id = 5)
